...
import {
  TITLE,
  PROGRAMS
} from "~/cardListKeys/benefitMileage";
import {CARDS_LISTS_ROUTE} from "@/constants_routes";
...
export const getters = {
  getList: state => list_key => {
    return JSON.parse(JSON.stringify(state))['benefitMileageLists'][list_key];
  }
};

export const actions = {
  [UPDATE_STATE_ACTION]: async ({ commit }, list_key) => {
    const key = `${BMSC}-benefits-mileage-${list_key}`;
    try {
...
export const mutations = {
  [UPDATE_STATE_MUTATION]: (state, res) => {
    state.benefitMileageLists[res.list] = res.data;
  }
};
