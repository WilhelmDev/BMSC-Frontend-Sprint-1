...
import {BENEFITS, IMAGE, INFORMATION_PANEL, TITLE, COMPARE} from "~/cardListKeys/investmentDetails";
export const state = () => ({
  investmentDetailsLists: {
    [TITLE]: [],
    [IMAGE]: [],
    [BENEFITS]: [],
    [INFORMATION_PANEL]: [],
...
export const getters = {
  getList: state => list_key => {
    let res = JSON.parse(JSON.stringify(state))['investmentDetailsLists'][list_key];
    return res;
  }
};
  ...
  [UPDATE_STATE_ACTION]: async ({ commit },[key,list_key,context]) => {
    try {
      let response = await executeRequest({
        url: `${CARDS_LISTS_ROUTE}/key/${BMSC}-investment-details-${key}-${list_key}`
      },true,context);
...
export const mutations = {
  [UPDATE_STATE_MUTATION]: (state, res) => {
    state.investmentDetailsLists[res.list] = res.data;
  }
};
