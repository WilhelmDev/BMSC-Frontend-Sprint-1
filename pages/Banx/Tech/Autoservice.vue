...
export default {
  ...
  computed: {
    titleData: function () {
      let res = {
        items: this.$store.getters["banx/getList"](TECH_AUTOSERVICE_TITLE)
      }
    ...
    programsData: function () {
      let sectionList = this.$store.getters["banx/getList"](TECH_AUTOSERVICE_PROGRAMS);
      let res = {
        items: sectionList
      }
    ...
    benefits2Data: function () {
      let sectionList = this.$store.getters["banx/getList"](TECH_AUTOSERVICE_BENEFITS_2);
  ...
  async asyncData(context) {
    ...
    await context.store.dispatch(`banx/${UPDATE_STATE_ACTION}`, TECH_AUTOSERVICE_TITLE);
    await context.store.dispatch(`banx/${UPDATE_STATE_ACTION}`, TECH_AUTOSERVICE_PROGRAMS);
    await context.store.dispatch(`banx/${UPDATE_STATE_ACTION}`, TECH_AUTOSERVICE_BENEFITS_2);
    return header;
  },
