<template>
  <div>
    <v-container class="text-center">
      <OptionsPanel v-if="titleData.items && titleData.items.length > 0" :data="titleData.items[0]">
    ...
    />
    <DetailsCard
      v-if="sandBoxData && sandBoxData.items && sandBoxData.items.length > 0"
      :detailsData="sandBoxData.items[0]" class="my-16"
    />
    <Compare
      ...
      :onButtonClick="goToHref.bind(this,compareData.items[0])"
      class="py-16"
    />
    <MoreInfo
...
export default {
  ...
  async mounted() {
    try {
      ...
      //console.log('error trying to fetch banx - tech - apiMarket data', error);
    }
  },
  computed: {
    titleData: function () {
      let res = {
        items: this.$store.getters["banx/getList"](TECH_API_MARKET_TITLE)
      }
    ...
    cardsData: function () {
      let res = {
        items: this.$store.getters["banx/getList"](TECH_API_MARKET_CARDS)
      }
    ...
    sandBoxData: function () {
      let res = {
        items: this.$store.getters["banx/getList"](TECH_API_MARKET_SANDBOX)
      }
    ...
    compareData: function () {
      let res = {
        items: this.$store.getters["banx/getList"](TECH_API_MARKET_COMPARE)
      }
  ...
  async asyncData(context) {
    ...
    await context.store.dispatch(`banx/${UPDATE_STATE_ACTION}`, TECH_API_MARKET_TITLE);
    await context.store.dispatch(`banx/${UPDATE_STATE_ACTION}`, TECH_API_MARKET_CARDS);
    await context.store.dispatch(`banx/${UPDATE_STATE_ACTION}`, TECH_API_MARKET_SANDBOX);
    await context.store.dispatch(`banx/${UPDATE_STATE_ACTION}`, TECH_API_MARKET_COMPARE);
    return header;
  },
