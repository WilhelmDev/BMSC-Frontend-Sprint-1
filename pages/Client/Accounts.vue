  ...
  <div>
    <v-container class="bm-loans mx-auto">
     ...
     <div class="w-100 text-center">
        <v-btn depressed @click="goToSimulatorAccount" color="bm-medium">
          Ir a simula rendimiento<v-icon right class="icon-btn-arrow my-auto"> </v-icon>
        </v-btn>
      </div>
...
export default {
  ...
  computed: {
    titleData: function () {
      let res = {
        items: this.$store.getters["client/getList"](ACCOUNTS_TITLE)
      }
    ...
    cardsData: function () {
      let res = {
        items: this.$store.getters["client/getList"](ACCOUNTS_CARDS)
      }
      //console.log(ACCOUNTS_CARDS, res);
      return res;
    },
    benefitsData: function () {
      let sectionList = this.$store.getters["client/getList"](ACCOUNTS_BENEFITS);
    ...
    programsData: function () {
      let sectionList = this.$store.getters["client/getList"](ACCOUNTS_PROGRAMS);
    ...
    benefits2Data: function () {
      let sectionList = this.$store.getters["client/getList"](ACCOUNTS_BENEFITS_2);
      if(sectionList.length === 0) sectionList = generateEmptyModels(1);
    ...
    compareData: function () {
      let res = {
        items: this.$store.getters["client/getList"](ACCOUNTS_COMPARE)
      }
  ...
  methods: {
    ...
    goToSimulatorAccount() {
      this.$router.push({
        path: "/simulatePerformance"
      });
    }
  },
  async asyncData(context) {
    const header = await getHeader(context);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`, ACCOUNTS_TITLE);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`, ACCOUNTS_CARDS);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`, ACCOUNTS_BENEFITS);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`, ACCOUNTS_PROGRAMS);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`, ACCOUNTS_BENEFITS_2);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`, ACCOUNTS_COMPARE);
    return header;
  },
