        ...
        <template v-slot:bottom>
          <div class="px-3">
            <Button
              ...
              :color="'bm-light'"
              :onClick="goToHref"
              :data="{buttonHref:'https://cyco.bmsc.com.bo/'}"
            />
          </div>
          <div class="px-3 pb-2">
            <Button
...
export default {
  ...
  methods: {
    ...
    async loadCreditCardDetailsData() {
      await this.$store.dispatch(`creditCardDetails/${UPDATE_STATE_ACTION}`,[this.creditCardKey,TITLE, this]);
      await this.$store.dispatch(`creditCardDetails/${UPDATE_STATE_ACTION}`,[this.creditCardKey,BENEFITS, this]);
      await this.$store.dispatch(`creditCardDetails/${UPDATE_STATE_ACTION}`,[this.creditCardKey,PROGRAMS, this]);
      await this.$store.dispatch(`creditCardDetails/${UPDATE_STATE_ACTION}`,[this.creditCardKey,BENEFITS_2, this]);
      await this.$store.dispatch(`creditCardDetails/${UPDATE_STATE_ACTION}`,[this.creditCardKey,INFORMATION_PANEL, this]);
      await this.$store.dispatch(`creditCardDetails/${UPDATE_STATE_ACTION}`,[this.creditCardKey, COMPARE, this]);
    },
    goToHref(item) {
  ...
  computed: {
    ...
    titleData: function () {
      let res = {
        items: this.$store.getters["creditCardDetails/getList"](TITLE)
      }
    ...
    benefitsData: function () {
      let sectionList = this.$store.getters["creditCardDetails/getList"](BENEFITS);
    ...
    programsData: function () {
      let res = {
        items: this.$store.getters["creditCardDetails/getList"](PROGRAMS)
      }
      //console.log(PROGRAMS, res);
      return res;
    },
    benefits2Data: function () {
      let sectionList = this.$store.getters["creditCardDetails/getList"](BENEFITS_2);
    ...
    informationPanelData: function () {
      let res = {
        items: this.$store.getters["creditCardDetails/getList"](INFORMATION_PANEL)
      }
      // needed fallback
    ...
    compareData: function () {
      let res = {
        items: this.$store.getters["creditCardDetails/getList"](COMPARE)
      }
  ...
  watch: {
    $route (to, from){
      ...
      this.creditCardKey = key;
      this.loadCreditCardDetailsData();
      this.scrollToTop();
    }
  },
