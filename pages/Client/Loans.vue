...
export default {
  ...
  computed: {
    titleData: function () {
      let res = {
        items: this.$store.getters["client/getList"](LOANS_TITLE)
      }
    ...
    cardsData: function () {
      let res = {
        items: this.$store.getters["client/getList"](LOANS_CARDS)
      }
      //console.log(LOANS_CARDS, res);
      return res;
    },
    benefitsData: function () {
      let sectionList = this.$store.getters["client/getList"](LOANS_BENEFITS);
    ...
    programsData: function () {
      let sectionList = this.$store.getters["client/getList"](LOANS_PROGRAMS);
    ...
    benefits2Data: function () {
      let sectionList = this.$store.getters["client/getList"](LOANS_BENEFITS_2);
      if(sectionList.length === 0) sectionList = generateEmptyModels(1);
    ...
    compareData: function () {
      return {
        items: this.$store.getters["client/getList"](LOANS_COMPARE)
      }
    }
  },
  data() {
  ...
  async asyncData(context) {
    ...
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`,LOANS_TITLE);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`,LOANS_CARDS);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`,LOANS_BENEFITS);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`,LOANS_PROGRAMS);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`,LOANS_BENEFITS_2);
    await context.store.dispatch(`client/${UPDATE_STATE_ACTION}`, LOANS_COMPARE);
    return header;
  },
