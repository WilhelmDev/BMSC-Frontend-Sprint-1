...
export default {
  ...
  computed: {
    titleData: function () {
      let res = {
        items: this.$store.getters["pyme/getList"](INVESTMENT_TITLE)
      }
    ...
    cardsData: function () {
      let res = {
        items: this.$store.getters["pyme/getList"](INVESTMENT_CARDS)
      }
      //console.log(INVESTMENT_CARDS, res);
      return res;
    },
    benefitsData: function () {
      let sectionList = this.$store.getters["pyme/getList"](INVESTMENT_BENEFITS);
    ...
    programsData: function () {
      let sectionList = this.$store.getters["pyme/getList"](INVESTMENT_PROGRAMS);
    ...
    benefits2Data: function () {
      let sectionList = this.$store.getters["pyme/getList"](INVESTMENT_BENEFITS_2);
      if(sectionList.length === 0) sectionList = generateEmptyModels(1);
  ...
  async asyncData(context) {
    ...
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INVESTMENT_TITLE);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INVESTMENT_CARDS);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INVESTMENT_BENEFITS);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INVESTMENT_PROGRAMS);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INVESTMENT_BENEFITS_2);
    return header;
  },
