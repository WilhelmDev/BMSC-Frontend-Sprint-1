...
import {
  ...
  INSURANCE_CARDS, INSURANCE_PROGRAMS,
...
export default {
        ...
        items: [
          {
            ...
            buttonHref: "/insuranceComparison"
          }
        ]
      },
      benefitsConfig: {
  ...
  computed: {
    titleData: function () {
      let res = {
        items: this.$store.getters["pyme/getList"](INSURANCE_TITLE)
      }
    ...
    cardsData: function () {
      let res = {
        items: this.$store.getters["pyme/getList"](INSURANCE_CARDS)
      }
    ...
    reviewsData: function () {
      let res = {
        items: this.$store.getters["pyme/getList"](INSURANCE_REVIEWS_CAROUSEL)
      }
      //console.log(INSURANCE_REVIEWS_CAROUSEL, res);
      return res;
    },
    benefitsData: function () {
      let sectionList = this.$store.getters["pyme/getList"](INSURANCE_BENEFITS);
    ...
    programsData: function () {
      let sectionList = this.$store.getters["pyme/getList"](INSURANCE_PROGRAMS);
    ...
    benefits2Data: function () {
      let sectionList = this.$store.getters["pyme/getList"](INSURANCE_BENEFITS_2);
      if(sectionList.length === 0) sectionList = generateEmptyModels(1);
    ...
    compareData: function () {
      let res = {
        items: this.$store.getters["pyme/getList"](INSURANCE_COMPARE)
      }

      //console.log(INSURANCE_COMPARE, res);
      return res;
    },
  ...
  async asyncData(context) {
    ...
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INSURANCE_TITLE);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INSURANCE_CARDS);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INSURANCE_REVIEWS_CAROUSEL);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INSURANCE_BENEFITS);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INSURANCE_PROGRAMS);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INSURANCE_BENEFITS_2);
    await context.store.dispatch(`pyme/${UPDATE_STATE_ACTION}`,INSURANCE_COMPARE);
    return header;
  },
