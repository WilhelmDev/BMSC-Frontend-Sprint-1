  ...
  <div class="white-box pa-5 accountCupon">
            ...
            <div class="modal-container pb-3 height-cupon" style="border-radius: 5px">
              <v-row justify="space-between" no-gutters align="center">
                ...
                <v-col cols="1">
                  <v-row justify="end" no-gutters align="center">
                    <v-col cols="10">
                      <img :src="require('@/assets/images/close.svg')" :class="`comModal__close-icon`" @click="showModal = false"/>
                    </v-col>
                  </v-row>
                </v-col>
              </v-row>
              ...
              <v-row class="px-3">
                <v-col cols="12" lg="8" md="8" sm="12" class="py-0 pr-lg-10 pr-md-10 pr-4 pt-2">
                  <v-text-field outlined v-model="searchCoupon" :rules="[rules.required]" class="text-field" placeholder="Busca entre tus cupones" maxlength="50"></v-text-field>
                </v-col>
              </v-row>
              ...
              <div id="coupon-scroll" v-if="couponList.length>0" class="my-3">
                <v-row class="px-3">
                  <v-col v-for="(item, index) in couponList" v-if="index+1>(pages-1)*topItems && (index+1)<=pages*topItems" :key="index"
                         cols="6" lg="4" md="4" sm="6" class="py-2 border-row">
                    <div class="comModal__text-small text-right mx-auto" style="width: 100px" v-html="colorGreen(item)"></div>
                  </v-col>
                </v-row>
              </div>
              <p class="comModal__text-small grey-md-text px-3" v-if="couponList.length===0">No encontramos cupones existentes con el número ingresado.</p>
              <v-row justify="center" align="center" class="px-3">
                <v-col cols="12" class="px-0 py-0">
                  <v-pagination class="mt-0 mb-2 pb-0 cupon"
                    v-model="pages"
                    :length="stateLength"
                    :total-visible="7"
                    color="transparent"
                    circle next-icon="icon-right-arrow" prev-icon="icon-left-arrow"
                  ></v-pagination>
                </v-col>
              </v-row>
            </div>
    ...
    <v-radio-group v-model="inputData.document" :mandatory="true" row>
      <v-radio
        ...
        value="ci"
        class="small pr-3"
        color="bm-radio" @change="clear('ci')"
      ></v-radio>
    ...
    <form class="pb-5">
      <v-row v-if="inputData.document === 'ci'">
        <v-col cols="12" sm="8">
          ...
          <v-text-field required solo class="bm-text-field" hide-details
                        v-model="inputData.numberCI"
          ></v-text-field>
        </v-col>
        <v-col class="pl-sm-0" cols="6" sm="4">
          <label>Expedici&oacute;n</label>
          <v-select class="border-select"
      ...
      <v-row v-if="inputData.document === 'other'">
        <v-col cols="12" sm="8">
          ...
          <v-text-field required solo class="bm-text-field" hide-details
                        v-model="inputData.numberPassport"
          ></v-text-field>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12" sm="8" class="pt-1 pt-sm-0">
          ...
          <v-text-field required solo class="bm-text-field" hide-details
                        v-model="inputData.accountNumber"
          ></v-text-field>
        </v-col>
      </v-row>
    </form>
...
export default {
  ...
  data() {
    return {
      ...
      cities: ["BE","CB","CH","LP","OR","PA","PT","SC","TJ"],
      showModal: false,
      searchCoupon: '',
      rules: {
        required: value => {
