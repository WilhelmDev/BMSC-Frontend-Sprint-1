               ...
               <v-row align="center" justify="center">
                 <v-col cols="11">
                   <v-row justify="start" no-gutters>
                     <v-col cols="10">
                       <h2>La creacion del grupo familiar {{nameFam}} fue exitosa</h2>
                     </v-col>
                   </v-row>
                   <v-row justify="center" no-gutters style="margin-top: 10px">
                     <v-col cols="12">
                      <p :class="`${ns}__text`">
                        <strong>Tu codigo de familia es: {{ familyId }}</strong>
                      </p>
                       <p :class="`${ns}__text`">
                         {{ message }}
                       </p>
...
<script>
    export default {
        ...
        methods: {
            async inicialize() {
                const {data} = await axios(`${process.env.SERVICES_BASE_URL}/api/bmscfamily/family/parameters/5`)
                const { valor } = data
                this.message = valor
            },
            closeModal() {
